...
public:
  ...
  MutableArrayRef<ValueBase> getResults();
  ArrayRef<ValueBase> getResults() const {
    return const_cast<SILInstruction*>(this)->getResults();
  }

  // Useful for ArrayRefView.
  ...
  bool isIdenticalTo(const SILInstruction *RHS, OpCmp &&opEqual) const {
    ...
    auto lhsResults = getResultTypes(), rhsResults = RHS->getResultTypes();
    if (lhsResults.size() != rhsResults.size() ||
        !std::equal(lhsResults.begin(), lhsResults.end(), rhsResults.begin())) {
      return false;
    }
...
/// An instruction which produces a single value.
...
public:
  ...
  MutableArrayRef<ValueBase> getResults() {
    return MutableArrayRef<ValueBase>(this, 1);
  }
  ArrayRef<ValueBase> getResults() const {
...
public:
  ...
  ArrayRef<ValueBase> getResults() const = delete;
};
